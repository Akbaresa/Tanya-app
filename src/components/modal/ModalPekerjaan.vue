  ...
  <div class="relative p-4 w-full max-w-md max-h-full">
      ...
      <div class="relative bg-white rounded-lg shadow dark:bg-gray-700  border border-gray-500">
          ...
          <div class="p-4 md:p-5">
              <form class="space-y-4" action="#" @submit.prevent="postDataPekerjaan">
                  <div>
                      <label  class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Posisi</label>
                      <input v-model="formData.posisi" type="text" name="posisi" id="posisi" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" placeholder="posisi di perusahaan" required>
                  </div>
                  <div>
                      <label  class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Perusahaan</label>
                      <input v-model="formData.perusahaan" type="text" name="perusahaan" id="perusahaan" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" placeholder="nama perusahaan" required>
                  </div>
...
export default {
  ...
  methods:{
    ...
    async postDataPekerjaan(){
      const token = localStorage.getItem('token');
      axios.post('http://localhost:8091/api/pekerjaan', this.formData, {
        headers: {
          'X-API-TOKEN' : token
        }
      })
